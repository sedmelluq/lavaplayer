...
class DefaultYoutubeTrackDetails(
  ...
  private fun loadTrackInfo(): AudioTrackInfo {
    ...
    val temporalInfo = TemporalInfo.fromRawData(videoDetails["isLiveContent"].asBoolean(false), videoDetails["lengthSeconds"])
    val artworkUrl = ThumbnailTools.extractYouTube(videoDetails, videoId)

    return buildTrackInfo(videoId, videoDetails["title"].safeText, videoDetails["author"].safeText, temporalInfo, artworkUrl)
  }
  ...
  private fun buildTrackInfo(videoId: String, title: String, uploader: String, temporalInfo: TemporalInfo, artworkUrl: String): AudioTrackInfo {
  ...
  data class TemporalInfo(val isActiveStream: Boolean, val durationMillis: Long) {
    companion object {
      fun fromRawData(wasLiveStream: Boolean, durationSecondsField: JsonBrowser): TemporalInfo {
        ...
        return TemporalInfo(isActiveStream, DURATION_MS_UNKNOWN.takeIf { durationValue == 0L } ?: Units.secondsToMillis(durationValue))
      }
    }
  }
}
