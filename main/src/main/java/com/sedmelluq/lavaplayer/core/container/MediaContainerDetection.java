...
import com.sedmelluq.lavaplayer.core.tools.io.GreedyInputStream;
...
import static com.sedmelluq.lavaplayer.core.container.MediaContainerDetectionResult.unknownFormat;
...
public class MediaContainerDetection {
   ...
   */
  public MediaContainerDetection(MediaContainerRegistry containerRegistry, AudioInfoRequest request,
  ...
  private MediaContainerDetectionResult detectContainer(SeekableInputStream innerStream, boolean matchHints)
    ...
    for (MediaContainerProbe probe : containerRegistry.getAll()) {
      if (matchHints == probe.matchesHints(hints)) {
        innerStream.seek(0);
        MediaContainerDetectionResult result = checkContainer(probe, request, innerStream);
  ...
  private static MediaContainerDetectionResult checkContainer(
      MediaContainerProbe probe,
      AudioInfoRequest request,
      SeekableInputStream inputStream
  ) {
    try {
      return probe.probe(request, inputStream);
    } catch (Exception e) {
