...
public class Mp3TrackProvider implements AudioTrackInfoProvider, Closeable {
   ...
   */
  public Mp3TrackProvider(AudioPlaybackContext context, SeekableInputStream inputStream) {
  ...
  public void provideTrackInfo(AudioTrackInfoBuilder builder) {
    builder
        .with(coreTitle(getIdv3Tag(TITLE_TAG)))
        .with(coreAuthor(getIdv3Tag(ARTIST_TAG)))
        .with(coreLength(getDuration()));
  }
