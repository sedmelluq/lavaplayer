...
import com.sedmelluq.lavaplayer.core.tools.io.EmptyInputStream;
...
import static com.sedmelluq.lavaplayer.core.http.HttpClientTools.isSuccessWithContent;
...
public class PersistentHttpStream extends SeekableInputStream implements AutoCloseable {
  ...
  public void provideTrackInfo(AudioTrackInfoBuilder builder) {
    if (currentResponse != null) {
      collectIceCastHeaders(currentResponse, builder);
    }
  }

  private void collectIceCastHeaders(HttpResponse response, AudioTrackInfoBuilder builder) {
    builder
        .with(coreTitle(getHeaderValue(response, "icy-url")))
        .with(coreTitle(getHeaderValue(response, "icy-description")))
        .with(coreAuthor(getHeaderValue(currentResponse, "icy-name")));
  }
}
